```php
原贴地址(在码云):https://git.oschina.net/popgxm/codes/ejbn8h63oidsakuw9myx271
...
Class DB {
    ...
    public function __construct() {
		...
		$this->closemysql = $db_config["pconnect"];
        if($this->is_log){
    ...
    public function update( $table,$dataArray,$condition="") {
        ...
        $value = "";
		$jk = array_unique($dataArray);
        while(list($key,$val) = each($jk)){
        	$value .= "$key = "."'".$val."',";
		}
        $value .= substr( $value,0,-1);

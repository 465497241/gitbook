```php
原贴地址(在码云):https://git.oschina.net/635050/codes/dsrncvgkz0wxtj19fq56m
	...
	function get_goods($num=0,$cate_id=0)
	{
		...
		if($tempalte)
		{
			...
			foreach ($ju_list as $key => $ju)
        	{
				...
				$ju_list[$key]['group_desc']   = strip_tags($ju['group_desc']);
				if($ju['price'] > 0){
				...
				$ju_list[$key]['lefttime'] = $init->lefttime($tempalte['end_time']);
        	}
		}
		return $ju_list;
	}
